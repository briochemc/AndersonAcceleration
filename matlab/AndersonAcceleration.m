...
% BP comment: the call we use is AndersonAcceleration(@g_age, x, [], histParams, restartfile);
% so nargin == 5
...
% BP: Below is some obscure branching for argument validation
% TODO BP: Refactor using modern MATLAB arg validation syntax

if nargin < 2 % BP: never hit for us
...
if nargin > 6 && ~isempty(aa)  % BP: never hit for us
    reuseAA = 1;
...
if nargin < 5  % BP: never hit for us
    disp('No restart')
    doRestart = 0;
    doRestartWithFile = 0;
    doRestartWithStruct = 0;
else % BP: always hit because nargin == 5
    if isempty(restartFile) && nargin < 7
        % BP: never hit for us because restartFile
        % is a defined string.
        doRestart = 0;
        disp('No restart')
    else % BP: always hit
        if ~isempty(restartFile) && reuseAA % BP: never hit for us (reuse AA set above)
            % FIXME: Isn't the check for ~isempty(restartFile) unnecessary??
            error('ERROR: Cannot specify both restart file and aa struct')
        end

        if nargin > 6  % AA takes priority % BP: never hit for us (nargin == 5)
            doRestart = 1;
            doRestartWithFile = 0;
            % doRestartWithStruct = 1;
            disp('Restart with AA')
        elseif ~isempty(restartFile) % BP: always hit
            doRestart = 1;
            doRestartWithFile = 1;
            % doRestartWithStruct = 0;
            disp('Restart with file')
            if nargin < 6 % always hit (nargin == 5)
                fileSuff = [];
            end
        else
...
if ~doRestartWithFile || (doRestartWithFile && ~exist(restartFile,'file'))
    ...
    if iniAA
        aa.x = x;
        aa.mMax = min(50, size(aa.x,1)); % TODO clarify what each param is exactly
        aa.itmax = 100;
        aa.atol = 1.e-10;
        aa.rtol = 1.e-10;
        aa.droptol = 1.e10;
...
if (doRestartWithFile && exist(restartFile,'file'))
    disp(['Loading restart file ' restartFile '...'])
    load(restartFile, 'aa')
...
for iter = aa.iter0:aa.itmax
    if exist('killaa','file') % CHECK BP: Is this ever hit?
